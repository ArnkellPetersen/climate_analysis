...
def histo2D(df):
    ...
    xrange = max_x - min_x
    yrange = max_y - min_y 
    H, xedges, yedges = np.histogram2d(df['DB'], df['RH'], bins=[xrange,yrange], range=[[min_x,max_x],[min_y,max_y]])
    ...
    mapper_main = LinearColorMapper(palette=Turbo256, low=0, high=max(H_flat))

    f5=figure(height =400, width = 800, x_axis_label='Dry-Bulb Temperature (C)', y_axis_label='Relative Humidity (%)',
            x_range=factors_x, y_range=factors_y,tools=tools,toolbar_location='above')
    f5.rect(x='x',y='y', line_color='black',width=1,width_units='data', height=1,fill_color={'field': 'hist', 'transform': mapper_main},source=source)
    ...
    #1d histogram y
    hist_y, edges_y = np.histogram(df['RH'],bins = yrange, range = [min_y,max_y])
    ...
    #1d histogram x
    hist_x, edges_x = np.histogram(df['DB'],bins = xrange, range = [min_x,max_x])
