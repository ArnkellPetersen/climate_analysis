...
def monthlyGraph(df):
    ...
    f3=figure(title='Monthly Stats',y_axis_label='Dry-Bulb Temperature (°C)',x_axis_type='datetime',width=950,height=500, tools=tools)
    ...
    for i in range(12):
        ...
        day.append(1)
        month.append(i+1)
        month_values=df[df['Month']==i+1]['DB']
    ...
    newdf=pd.DataFrame({'Year':year,
                        ...
                        'DB_25pc': month_25pc,
                        'DB_75pc': month_75pc,
                        'DB_75pc_centre': month_75pc_centre,
                        'DB_75pc_height': month_75pc_height,
                        'DB_25pc_centre': month_25pc_centre,
                        'DB_25pc_height': month_25pc_height})
    ...
    #adjust left axis
    f3.y_range=Range1d(start=newdf['DB_low'].min()-2, end=newdf['DB_high'].max()+2)

    #add low line
    g1 = bkm.Line(x='dates', y='DB_low',line_color='blue',line_alpha=0.5,line_width=3)
    g1_r = f3.add_glyph(source_or_glyph=source3, glyph=g1)
    f3.circle(x='dates',y='DB_low',size=8,color='blue',source=source3)

    #add high line
    g2 = bkm.Line(x='dates', y='DB_high',line_color='orange',line_alpha=0.5,line_width=3)
    g2_r = f3.add_glyph(source_or_glyph=source3, glyph=g2)
    f3.circle(x='dates',y='DB_high',size=8,color='orange',source=source3)

    #add mean line
    g3 = bkm.Line(x='dates', y='DB_mean',line_color='green',line_alpha=0.5,line_width=3)
    g3_r = f3.add_glyph(source_or_glyph=source3, glyph=g3)

    #add bars
    g4=bkm.Rect(x='dates',y='DB_mean',width=20,height=0.5,height_units='data',
                width_units='screen',fill_color='green',line_color='green')
    g4_r = f3.add_glyph(source_or_glyph=source3, glyph=g4)
    g5=bkm.Rect(x='dates',y='DB_75pc_centre',width=20,height='DB_75pc_height',
    ...
    g6=bkm.Rect(x='dates',y='DB_25pc_centre',width=20,height='DB_25pc_height',
                height_units='data',width_units='screen',fill_color='green',
                fill_alpha=0.3,line_color='green',line_alpha=0.6,line_width=3)
    g6_r = f3.add_glyph(source_or_glyph=source3, glyph=g6)
    g7=bkm.Circle(x='dates',y='DB_25pc',size=8,fill_color='green',line_color='green')
    g7_r = f3.add_glyph(source_or_glyph=source3, glyph=g7)
    g8=bkm.Circle(x='dates',y='DB_75pc',size=8,fill_color='green',line_color='green')
    g8_r = f3.add_glyph(source_or_glyph=source3, glyph=g8)

    #add hover tools
    g1_hover = bkm.HoverTool(renderers=[g1_r], tooltips=[('Month', '@dates{%B}'),('Min Temp', '@DB_low{0.0}°C')],formatters={'@dates':'datetime'})
    g2_hover = bkm.HoverTool(renderers=[g2_r], tooltips=[('Month', '@dates{%B}'),('Max Temp', '@DB_high{0.0}°C')],formatters={'@dates':'datetime'})
    g3_hover = bkm.HoverTool(renderers=[g3_r], tooltips=[('Month', '@dates{%B}'),('Average Temp', '@DB_mean{0.0}°C')],formatters={'@dates':'datetime'})
    g7_hover = bkm.HoverTool(renderers=[g7_r], tooltips=[('Month', '@dates{%B}'),('25th percentile', '@DB_25pc{0.0}°C')],formatters={'@dates':'datetime'})
    g8_hover = bkm.HoverTool(renderers=[g8_r], tooltips=[('Month', '@dates{%B}'),('75th percentile', '@DB_75pc{0.0}°C')],formatters={'@dates':'datetime'})
